...
class Logger:
  ...
  logger_name = app_cfg.LOGGER_NAME
  ...
  def create(verbosity=4, logfile=None):
    ...
    if 'colorlog' in sys.modules and os.isatty(2):
      cformat = '%(log_color)s' + app_cfg.LOGFILE_FORMAT
      f = colorlog.ColoredFormatter(cformat, date_format,
        log_colors = { 'DEBUG'   : 'yellow',       'INFO' : 'white',
        'WARNING' : 'bold_yellow', 'ERROR': 'bold_red',
        'CRITICAL': 'bold_red' })
    else:
      f = logging.Formatter(app_cfg.LOGFILE_FORMAT, date_format)
    
    # logger = logging.getLogger(Logger.logger_name)
    logger = logging.getLogger(app_cfg.LOGGER_NAME)
  ...
  @staticmethod
  def getLogger():
    return logging.getLogger(app_cfg.LOGGER_NAME)